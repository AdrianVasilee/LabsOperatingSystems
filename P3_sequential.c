...
typedef struct {
  ...
  int bytesToRead;  // any other info you might want to pass
} ThreadInfo;

int main(int argc, char* argv[]) {
  if (argc != 3) {
    printf(
        "Incorrect usage. Args needed: histogram pathToImage "
        "pathToHistogramOut\n");
    _exit(1);
  }
  ...
  lseek(fd, nBytesHeader, SEEK_SET);  // Move the cursor to the start of the
                                      // data segment For the
  for (int i = 0; i < nPixels; i += BUFF_SIZE) {
    nBytesRead = read(fd, buffer, BUFF_SIZE);
    if (nBytesRead <= 0) {
      printf(
          "Unexpected error: the file ended before all pixels where read.\n");
      _exit(1);
    }
    for (int i = 0; i < nBytesRead; i++) histogram[buffer[i]]++;
  }
  close(fd);
